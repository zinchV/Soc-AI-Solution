...
SIMULATED_ASSETS = [
    ...
    {"id": 12, "hostname": "EXCHANGE-01", "ip_address": "10.0.10.20", "asset_type": "server", "os": "Windows Server 2019", "software": ["windows", "exchange", "office"], "criticality": "critical", "owner": "IT Ops"},
    {"id": 13, "hostname": "VPN-GATEWAY-01", "ip_address": "10.0.0.30", "asset_type": "security", "os": "Ivanti Connect Secure 22.4", "software": ["ivanti", "pulse", "connect-secure"], "criticality": "critical", "owner": "Security"},
    {"id": 14, "hostname": "CITRIX-ADC-01", "ip_address": "10.0.0.35", "asset_type": "network", "os": "Citrix ADC 13.1", "software": ["citrix", "netscaler", "adc"], "criticality": "critical", "owner": "Network Team"},
    {"id": 15, "hostname": "VMWARE-VCENTER-01", "ip_address": "10.0.20.10", "asset_type": "server", "os": "VMware vCenter 8.0", "software": ["vmware", "vcenter", "esxi"], "criticality": "critical", "owner": "Infrastructure"},
    {"id": 16, "hostname": "CONFLUENCE-01", "ip_address": "10.0.3.40", "asset_type": "server", "os": "Ubuntu 22.04", "software": ["atlassian", "confluence", "java"], "criticality": "high", "owner": "DevOps"},
    {"id": 17, "hostname": "APACHE-WEB-01", "ip_address": "10.0.1.20", "asset_type": "server", "os": "RHEL 8", "software": ["apache", "httpd", "openssl"], "criticality": "high", "owner": "Web Team"},
    {"id": 18, "hostname": "CISCO-ASA-01", "ip_address": "10.0.0.40", "asset_type": "network", "os": "Cisco ASA 9.18", "software": ["cisco", "asa", "firepower"], "criticality": "critical", "owner": "Network Team"},
]


PRODUCT_SOFTWARE_MAP = {

     # Fortinet - matches "Fortinet" vendor and "FortiOS", "FortiGate" products
    'fortios': ['fortios', 'fortigate'], 
    'fortigate': ['fortios', 'fortigate'],
    'fortinet': ['fortios', 'fortigate'],
    'fortiproxy': ['fortios', 'fortigate'],
    'fortiswitchmanager': ['fortios', 'fortigate'],

     # Palo Alto Networks - matches "Palo Alto Networks" vendor and "PAN-OS" products
    'panos': ['panos', 'paloalto'], 
    'pan-os': ['panos', 'paloalto'],
    'palo alto': ['panos', 'paloalto'],
    'palo alto networks': ['panos', 'paloalto'],
    'globalprotect': ['panos', 'paloalto'],
    'expedition': ['panos', 'paloalto'],

     # SonicWall - matches "SonicWall" vendor
    'sonicwall': ['sonicwall', 'sma1000'], 
    'sma100': ['sonicwall', 'sma1000'],
    'sma1000': ['sonicwall', 'sma1000'],
    'sonicos': ['sonicwall', 'sma1000'],


     # Microsoft - matches "Microsoft" vendor and Windows products
    'microsoft': ['windows', 'office', 'active-directory'],
    'windows': ['windows', 'active-directory'],
    'office': ['office'],
    'exchange': ['windows', 'office'],
    'outlook': ['office'],
    'sharepoint': ['windows', 'office'],
    'active directory': ['active-directory', 'windows'],
    '.net': ['windows'],

    # GitLab - matches "GitLab" vendor
    'gitlab': ['gitlab'],



    # Ivanti - matches "Ivanti" vendor and various products
    'ivanti': ['ivanti', 'epmm', 'mobileiron'],
    'epmm': ['ivanti', 'epmm', 'mobileiron'],
    'mobileiron': ['ivanti', 'epmm', 'mobileiron'],
    'pulse secure': ['ivanti', 'epmm'],
    'connect secure': ['ivanti', 'epmm'],
    'policy secure': ['ivanti', 'epmm'],
    'endpoint manager': ['ivanti', 'epmm'],
    'avalanche': ['ivanti', 'epmm'],
    'neurons': ['ivanti', 'epmm'],


     # SolarWinds - matches "SolarWinds" vendor
    'solarwinds': ['solarwinds', 'whd'],
    'orion': ['solarwinds'],
    'serv-u': ['solarwinds'],
    'web help desk': ['solarwinds', 'whd'],


    # Web servers / Common software
    'nginx': ['nginx'],
    ...
    'vcenter': ['vmware', 'esxi'],
     ...
     # Cisco
    'cisco': ['cisco'],
    'ios': ['cisco'],  # Cisco IOS, not Apple iOS
    ...
    'openssl': ['openssl'],
     ...
     # Citrix - frequently in CISA KEV
    'citrix': ['citrix', 'netscaler', 'adc'],
    'netscaler': ['citrix', 'netscaler', 'adc'],
    'citrix adc': ['citrix', 'netscaler', 'adc'],
    'citrix gateway': ['citrix', 'netscaler', 'adc'],


    # Atlassian - Confluence, Jira frequently exploited
    ...
    # Cisco ASA/Firepower
    'asa': ['cisco', 'asa', 'firepower'],
    'firepower': ['cisco', 'asa', 'firepower'],
    'adaptive security appliance': ['cisco', 'asa'],

    # F5 BIG-IP (commonly exploited)
    'f5': ['f5', 'bigip'],
    'big-ip': ['f5', 'bigip'],
    'bigip': ['f5', 'bigip'],
    ...
    'manageengine': ['zoho', 'manageengine'],

    # Progress/MOVEit
    'progress': ['moveit', 'progress'],
    'moveit': ['moveit', 'progress'],
        ...
        if kev_count == 0:
            ...
            if latest and latest.created_at:
                ...
                if age > timedelta(hours=24):
                    ...
                    print(f"✅ KEV data is fresh ({age.total_seconds()/3600:.1f} hours old) - {kev_count} CVEs in database")
            else:
...
async def fetch_kevs_from_cisa(db: Session):
    ...
    try:
        ...
        vulnerabilities = data.get('vulnerabilities', [])[:50]  # Get top 50 (increased from 20)
        saved_count = 0
        skipped_count = 0
        ...
        print(f"✅ KEV data loaded: {total_count} total CVEs ({saved_count} new, {skipped_count} already existed)")
...
async def refresh_threat_intel(force_fresh: bool = False, db: Session = Depends(get_db)):
    """Refresh threat intel from CISA KEV feed
    
    Args:
        force_fresh: If True, clear all existing threat intel and detection rules first
    """
    try:
        # If force_fresh, clear existing data to start fresh
        if force_fresh:
            db.query(DetectionRule).delete()
            db.query(ThreatIntel).delete()
            db.commit()
            print("🗑️ Cleared all existing threat intel and rules for fresh start")
        
        async with httpx.AsyncClient(verify=False, timeout=30.0) as client:
            response = await client.get('https://www.cisa.gov/sites/default/files/feeds/known_exploited_vulnerabilities.json')
            data = response.json()
        
        vulnerabilities = data.get('vulnerabilities', [])[:50]  # Get top 50
        saved_count = 0
        skipped_count = 0
        ...
        return {
            'success': True, 
            'message': f'Loaded {total_count} CVEs ({saved_count} new, {skipped_count} already existed)', 
            'count': total_count, 
...
async def get_kevs(
    ...
    # Get all rules to check which CVEs have been acted upon
    all_rules = db.query(DetectionRule).all()
    rules_by_cve = {r.source_cve: r for r in all_rules}
    
    # CVEs that have been acted upon (approved, deployed, or rejected rules)
    acted_upon_cves = {
        r.source_cve for r in all_rules 
        if r.status in ['approved', 'deployed', 'rejected']
    }
    ...
    results = []
    for t in threats:
        # SKIP CVEs that have already been acted upon (have approved/deployed/rejected rules)
        if t.cve_id in acted_upon_cves:
            continue
...
async def get_hunts(status: Optional[str] = None, only_actionable: bool = True, db: Session = Depends(get_db)):
    """Get all hunts with optional status filter
    
    Args:
        status: Filter by specific status
        only_actionable: If True (default), only return hunts that haven't been acted upon
                        (excludes: executed, completed, dismissed)
    """
    query = db.query(Hunt)
    ...
    if status: 
        ...
        # Only show hunts that need action: recommended, pending_approval, approved (not yet executed)
        # Exclude: executed, completed, dismissed
        query = query.filter(Hunt.status.in_(['recommended', 'pending_approval', 'approved']))
...
async def get_all_hunt_results(only_actionable: bool = True, db: Session = Depends(get_db)):
    """Get all hunt results
    
    Args:
        only_actionable: If True (default), only return results that haven't been escalated
    """
    query = db.query(HuntResult)
    
    if only_actionable:
        # Only show results that need action (not yet escalated)
        query = query.filter(HuntResult.escalated == False)
    
    results = query.order_by(HuntResult.executed_at.desc()).all()
...
@app.delete('/api/v1/hunts/clear')
async def clear_hunts(db: Session = Depends(get_db)):
    """Clear all hunts and hunt results to allow fresh generation"""
    try:
        # Delete hunt results first (foreign key)
        result_count = db.query(HuntResult).delete()
        # Delete pending actions related to hunts
        db.query(PendingAction).filter(PendingAction.action_type.in_(['execute_hunt', 'approve_hunt'])).delete(synchronize_session=False)
        # Delete hunts
        hunt_count = db.query(Hunt).delete()
        db.commit()
        return {
            'success': True,
            'message': f'Cleared {hunt_count} hunts and {result_count} results'
        }
    except Exception as e:
        db.rollback()
        return {'success': False, 'error': str(e)}
...
async def generate_hunts(use_llm: bool = False, fresh: bool = False, db: Session = Depends(get_db)):
    """Generate threat hunts from threat intelligence using AI
    
    Args:
        use_llm: Use LLM for intelligent hunt generation (default: False)
        fresh: Clear existing hunts and regenerate from scratch (default: False)
    """
    # If fresh=True, clear existing hunts first
    if fresh:
    ...
    # Get threat intel - ORDER BY LATEST FIRST
    threats = db.query(ThreatIntel).order_by(ThreatIntel.date_added.desc()).all()
    
    new_threats = [t for t in threats if t.cve_id not in existing_cves]
    
    # Prioritize by CVE year (2026 > 2025 > 2024 etc) and ransomware use
    def priority_sort(threat):
        # Extract year from CVE-YYYY-XXXXX
        try:
            year = int(threat.cve_id.split('-')[1])
        except:
            year = 2000
        # Prioritize: higher year, ransomware use
        return (
            year,  # Higher year = more recent
            1 if threat.ransomware_use else 0
        )
    
    new_threats.sort(key=priority_sort, reverse=True)
...
# Import VMG agent tools
from soc_agent.vmg_agent import (
...
# In-memory storage for vulnerabilities (for POC - in production would be in database)
VMG_VULNERABILITIES = []
VMG_LAST_SCAN = None

@app.post('/api/v1/vmg/scan')
async def run_vmg_scan():
    """
    Run a vulnerability scan (simulated Tenable scanner).
    Processes results through AI risk scoring engine.
    """
    global VMG_VULNERABILITIES, VMG_LAST_SCAN
    
    # Run simulated scan
    scan_result = simulate_tenable_scan()
    
    if not scan_result.get("success"):
        return {"success": False, "error": "Scan failed"}
    
    # Process each vulnerability through AI risk scoring
    processed_vulns = []
    
    for finding in scan_result.get("findings", []):
        # Get asset context
        asset = next(
            (a for a in SIMULATED_ASSETS if a["ip_address"] == finding["ip_address"]),
            {"criticality": "medium", "asset_type": "server"}
        )
        
        # Calculate AI risk score
        risk_result = calculate_ai_risk_score(
            cvss_score=finding["cvss_score"],
            asset_criticality=asset.get("criticality", "medium"),
            asset_type=asset.get("asset_type", "server"),
            in_cisa_kev=finding.get("in_cisa_kev", False),
            exploit_available=finding.get("exploit_available", False),
            epss_score=finding.get("epss_score", 0.1)
        )
        
        # Combine finding with risk score
        processed_vuln = {
            **finding,
            "asset_criticality": asset.get("criticality", "medium"),
            "asset_type": asset.get("asset_type", "server"),
            "asset_owner": asset.get("owner", "Unknown"),
            "ai_risk_score": risk_result["ai_risk_score"],
            "priority": risk_result["priority"],
            "priority_order": risk_result["priority_order"],
    ...
    VMG_VULNERABILITIES = prioritize_vulnerabilities(processed_vulns)
    VMG_LAST_SCAN = scan_result["scan_time"]
    
    # Calculate stats
    stats = get_vmg_dashboard_stats(VMG_VULNERABILITIES)
    ...
    return {
        ...
        "total_vulnerabilities": len(VMG_VULNERABILITIES),
        "stats": stats,
        "message": f"Scan complete. Found {len(VMG_VULNERABILITIES)} vulnerabilities. {stats['fix_now_count']} require immediate attention."
    }


@app.get('/api/v1/vmg/vulnerabilities')
async def get_vmg_vulnerabilities(
    ...
    Returns AI-prioritized list.
    """
    vulns = VMG_VULNERABILITIES.copy()
    ...
    if priority:
        vulns = [v for v in vulns if v.get("priority", "").lower() == priority.lower()]
    
    if kev_only:
        vulns = [v for v in vulns if v.get("in_cisa_kev")]
    
    if asset:
        asset_lower = asset.lower()
        vulns = [v for v in vulns if 
                 asset_lower in v.get("hostname", "").lower() or
                 asset_lower in v.get("ip_address", "").lower()]
    
    if status:
        vulns = [v for v in vulns if v.get("status", "").lower() == status.lower()]
    
    return {
        "success": True,
        "count": len(vulns),
        "last_scan": VMG_LAST_SCAN,
        "vulnerabilities": vulns
    }


@app.get('/api/v1/vmg/dashboard')
async def get_vmg_dashboard():
    """
    Get vulnerability management dashboard statistics.
    """
    if not VMG_VULNERABILITIES:
        return {
            "success": True,
            "last_scan": None,
            "stats": {
            ...
            "top_risks": [],
            "message": "No scan data. Run a vulnerability scan first."
        }
    
    stats = get_vmg_dashboard_stats(VMG_VULNERABILITIES)
    
    # Get top 5 risks
    top_risks = VMG_VULNERABILITIES[:5]
    
    # Calculate remediation progress
    total = len(VMG_VULNERABILITIES)
    remediated = len([v for v in VMG_VULNERABILITIES if v.get("status") == "Remediated"])
    in_progress = len([v for v in VMG_VULNERABILITIES if v.get("status") == "In Progress"])
    ...
    return {
        ...
        "top_risks": [
            {
                ...
                "risk_reasoning": v.get("risk_reasoning", "")
            }
            for v in top_risks
        ],
        "remediation_progress": {
            ...
            "open": total - remediated - in_progress,
            "percentage": round((remediated / total) * 100, 1) if total > 0 else 0
        }
    }


@app.get('/api/v1/vmg/vulnerability/{cve_id}')
async def get_vmg_vulnerability_detail(cve_id: str):
    """
    Get detailed information about a specific vulnerability including remediation guidance.
    """
    vuln = next((v for v in VMG_VULNERABILITIES if v["cve_id"] == cve_id), None)
    
    if not vuln:
        raise HTTPException(404, f"Vulnerability {cve_id} not found")
    
    # Get remediation guidance
    remediation = get_remediation_guidance(
        cve_id=cve_id,
...
@app.post('/api/v1/vmg/vulnerability/{cve_id}/assign')
async def assign_vmg_vulnerability(cve_id: str, assigned_to: str, due_date: Optional[str] = None):
    """
    Assign a vulnerability to a team/person for remediation.
    """
    global VMG_VULNERABILITIES
    
    for i, vuln in enumerate(VMG_VULNERABILITIES):
        if vuln["cve_id"] == cve_id:
            VMG_VULNERABILITIES[i]["assigned_to"] = assigned_to
            VMG_VULNERABILITIES[i]["due_date"] = due_date
            VMG_VULNERABILITIES[i]["status"] = "Assigned"
            
            return {
                "success": True,
                "message": f"{cve_id} assigned to {assigned_to}",
                "vulnerability": VMG_VULNERABILITIES[i]
            }
    
    raise HTTPException(404, f"Vulnerability {cve_id} not found")


@app.post('/api/v1/vmg/vulnerability/{cve_id}/status')
async def update_vmg_vulnerability_status(cve_id: str, status: str, notes: Optional[str] = None):
    """
    Update the remediation status of a vulnerability.
    Valid statuses: Open, Assigned, In Progress, Remediated, Accepted Risk, False Positive
    """
    global VMG_VULNERABILITIES
    
    valid_statuses = ["Open", "Assigned", "In Progress", "Remediated", "Accepted Risk", "False Positive"]
    if status not in valid_statuses:
        raise HTTPException(400, f"Invalid status. Must be one of: {valid_statuses}")
    
    for i, vuln in enumerate(VMG_VULNERABILITIES):
        if vuln["cve_id"] == cve_id:
            VMG_VULNERABILITIES[i]["status"] = status
            if notes:
                VMG_VULNERABILITIES[i]["remediation_notes"] = notes
            if status == "Remediated":
                VMG_VULNERABILITIES[i]["remediated_at"] = datetime.utcnow().isoformat()
            
            return {
                "success": True,
                "message": f"{cve_id} status updated to {status}",
                "vulnerability": VMG_VULNERABILITIES[i]
            }
...
@app.get('/api/v1/vmg/kev-correlation')
async def get_kev_correlation():
    """
    Get vulnerabilities that are in CISA KEV (actively exploited).
    These should be prioritized for immediate remediation.
    """
    kev_vulns = [v for v in VMG_VULNERABILITIES if v.get("in_cisa_kev")]
    
    return {
        "success": True,
        "count": len(kev_vulns),
        "message": f"⚠️ {len(kev_vulns)} vulnerabilities are actively exploited (CISA KEV)",
