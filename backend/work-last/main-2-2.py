...
PRODUCT_SOFTWARE_MAP = {
    ...
    'cisco': ['cisco', 'ios'], 'microsoft': ['windows', 'office', 'active-directory'],
...
def check_exposure_for_product(vendor: str, product: str) -> dict:
    ...
    Used by both the exposure endpoint and batch KEV check.
    """
    search_terms = []
    ...
    if not search_terms:
        search_terms = [vendor.lower(), product.lower()]
...
async def create_detection_rule(cve_id: str, db: Session = Depends(get_db)):
    ...
    Uses MITRE mapping based on vulnerability description.
    """
    ...
    # Get MITRE mapping from description
...
status: experimental
description: |
    Detection rule for {cve_id}: {threat.vulnerability_name or threat.product}
    {threat.description[:300] if threat.description else ''}
references:
...
detection:
    selection_product:
        cs-uri-query|contains:
            - '{threat.product.lower() if threat.product else "unknown"}'
    selection_exploit:
        cs-uri-query|contains:
            - 'exploit'
            - 'payload'
            - '../'
            - 'cmd='
    condition: selection_product or selection_exploit
falsepositives:
    - Legitimate administrative activity
    - Security scanning tools
level: high"""
    
    # Generate KQL query for Sentinel
    product_lower = (threat.product or 'unknown').lower()
    vendor_lower = (threat.vendor or 'unknown').lower()
    
    kql = f"""// Detection Rule for {cve_id}
// {threat.vendor} - {threat.product}
// MITRE ATT&CK: {tactic} - {technique} ({technique_name})
// Generated by SOC AI Tool
...
// Alternative: Check CommonSecurityLog for network-based detection
// CommonSecurityLog
// | where TimeGenerated > ago(lookback)
// | where RequestURL has_any (product_keywords)
//    or DestinationHostName has_any (product_keywords)
// | project TimeGenerated, SourceIP, DestinationIP, RequestURL, Activity
"""
    
    # Create the rule
    rule = DetectionRule(
        title=f"Detect {cve_id} - {threat.vendor} {threat.product}",
        description=f"Detection for {cve_id}: {threat.description[:500] if threat.description else 'No description'}",
        source_cve=cve_id,
        severity='high' if threat.severity == 'critical' else threat.severity or 'high',
        sigma_rule=sigma,
        kql_query=kql,
        mitre_techniques=json.dumps([technique]),
        status='pending'
    )
    db.add(rule)
    db.commit()
