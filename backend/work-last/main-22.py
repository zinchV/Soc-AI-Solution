...
async def check_exposure(vendor: str, product: str):
    ...
    Calls the threat_intel_agent's check_asset_exposure tool directly.
    """
    try:
        # Import the tool function directly
        from soc_agent.threat_intel_agent.tools import check_asset_exposure
        
        # Call the tool function
        result = check_asset_exposure(vendor, product)
        ...
        return {
            ...
            'exposed_count': result.get('exposed_count', 0),
        ...
        # Fallback if tools not available - return simulated data
        print(f"Warning: Could not import check_asset_exposure tool: {e}")
        return {
            'success': True,
            'vendor': vendor,
            'product': product,
            ...
            'exposed_count': 0,
            'assets': [],
            'recommendation': 'Tool not available - check installation',
            'error': str(e)
        }
    except Exception as e:
        return {
