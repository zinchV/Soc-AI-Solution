...
async function loadThreatIntel() {
    try {
        // Fetch KEVs with exposure check enabled
        ...
        if (badge) {
            // Show count of exposed CVEs without coverage
            const actionable = threatIntelData.filter(c => 
                c.is_exposed && !c.has_local_rule
            ).length;
            badge.textContent = actionable > 0 ? actionable : threatIntelData.length;
        }
    } catch (error) {
...
async function loadPendingActions() {
    try {
        ...
        const approvedRulesResponse = await fetch(`${API_BASE}/rules?status=approved`);
        const approvedRules = await approvedRulesResponse.json();
...
async function loadPendingHunts() {
    try {
        // Load hunts that are in 'new' or 'recommended' status (not yet implemented)
        const response = await fetch(`${API_BASE}/hunts?status=recommended`);
        const data = await response.json();
        ...
        if (hunts.length === 0) {
            container.innerHTML = '<p class="empty-state">No hunt recommendations. Click "Generate Hunts" to create AI-powered hunt hypotheses from threat intelligence.</p>';
            return;
        }
...
async function loadHuntResults() {
    try {
        ...
        if (results.length === 0) {
            container.innerHTML = '<p class="empty-state">No hunt results yet. Approve and execute hunts to see findings.</p>';
            return;
        }
            ...
            return `
                <div class="hunt-result-card ${severityClass}">
                    ...
                    <div class="hunt-result-actions">
                        ${result.escalated ? `
                            <span class="escalated-badge">✅ Escalated to Incident #${result.incident_id}</span>
                        ` : hasFindings ? `
                            <button class="btn btn-sm btn-warning" onclick="escalateHuntResult(${result.id})">⚠️ Escalate to Incident</button>
                        ` : ''}
                    </div>
                </div>
            `;
...
async function generateHunts() {
    showLoading('AI is analyzing threat intelligence and generating hunt hypotheses...');
    try {
        ...
        if (data.success) {
            ...
            if (count > 0) {
                showSuccess(`🎯 AI generated ${count} hunt recommendations based on CISA KEV threat intelligence`);
            } else {
...
async function loadThreatIntelPendingRules() {
    try {
        const response = await fetch(`${API_BASE}/detection-rules`);
        const data = await response.json();
        ...
        container.innerHTML = pendingRules.map(rule => `
            <div class="pending-item rule-item">
                ...
                <div class="pending-item-meta">
                    <span>📌 ${rule.related_cve || 'N/A'}</span>
                    <span>🎯 ${rule.mitre_technique || 'N/A'}</span>
                </div>
