...
function renderThreatIntelList() {
    ...
    // Sort: exposed + needs action first, then exposed + has rule, then non-exposed
    const sorted = [...threatIntelData].sort((a, b) => {
        // Priority: exposed without rule > exposed with rule > not exposed
        const scoreA = (a.is_exposed ? 2 : 0) + (!a.has_local_rule && a.is_exposed ? 1 : 0);
        const scoreB = (b.is_exposed ? 2 : 0) + (!b.has_local_rule && b.is_exposed ? 1 : 0);
    ...
    container.innerHTML = sorted.map(cve => {
        ...
        if (hasLocalRule) {
            if (isDeployed) {
                ...
                createButtonTitle = 'Rule already deployed to Sentinel';
        ...
        return `
            <div class="cve-card ${isDeployed ? 'has-rule' : ''} ${isExposed ? 'is-exposed' : ''}" 
                ...
                <div class="cve-actions" onclick="event.stopPropagation()">
                    ...
                    <button class="${createButtonClass}" 
                            ...
                            onclick="${createDisabled ? '' : `createRule('${cve.cve_id}')`}">
                        ${createButtonText}
                    </button>
                </div>
            </div>
...
function renderExposureSummary() {
    ...
    const needAction = exposed.filter(c => !c.has_local_rule);
    ...
    if (exposed.length === 0) {
        ...
        listContainer.innerHTML = exposed.map(cve => {
            ...
            const isPending = hasRule && !isDeployed;
            ...
            return `
                <div class="exposed-cve-item ${isDeployed ? 'covered' : isPending ? 'pending' : 'needs-action'}" 
