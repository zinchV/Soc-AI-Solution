...
async function loadPendingActions() {
    try {
        ...
        const pendingRules = (pendingRulesData.rules || []).map(r => ({...r, status: 'pending'}));
        const approvedRules = (approvedRulesData.rules || []).map(r => ({...r, status: 'approved'}));
        const rules = [...pendingRules, ...approvedRules];
        
        const totalCount = actions.length + rules.length;
        ...
        const pendingRulesHtml = rules.filter(r => r.status === 'pending').map(r => `
        ...
        // Render approved rules (ready for deployment)
        const approvedRulesHtml = rules.filter(r => r.status === 'approved').map(r => `
            <div class="action-card priority-high" style="border-left: 3px solid #22c55e;">
                <div class="action-header">
                    ...
                    <span class="badge" style="background: #22c55e; color: white;">READY TO DEPLOY</span>
                </div>
                <div class="action-body">
                    <strong>${r.title || 'Untitled Rule'}</strong>
                    <p>${(r.description || 'No description').substring(0, 100)}...</p>
                    ${r.source_cve ? `<p style="font-family: var(--font-mono); font-size: 0.8rem; color: #f59e0b;">CVE: ${r.source_cve}</p>` : ''}
                    <p style="font-size: 0.75rem; color: var(--text-muted);">Approved by: ${r.approved_by || 'Unknown'}</p>
                </div>
                <div class="action-buttons">
                    <button class="btn btn-sm" onclick="viewRule(${r.id})" style="background: rgba(99, 102, 241, 0.2); color: #a5b4fc;">
                        👁️ View Rule
                    </button>
                    <button class="btn btn-sm" onclick="deployRule(${r.id})" style="background: linear-gradient(135deg, #3b82f6, #1d4ed8); color: white;">
                        🚀 Deploy to Sentinel
                    </button>
                </div>
            </div>
    ...
    try {
        ...
        if (data.success) {
            const message = data.simulated 
                ? `✅ Rule deployed (simulated)\n\nSentinel Rule ID: ${data.sentinel_rule_id}`
                : `🚀 Rule deployed to Microsoft Sentinel!\n\nSentinel Rule ID: ${data.sentinel_rule_id}\n\nThe rule is now active in your Azure Sentinel workspace.`;
            
            alert(message);
            showSuccess('Rule deployed to Sentinel!');
            await loadPendingActions();
        } else {
